...
public class ChatWebSocketHandler extends TextWebSocketHandler {
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		if(message.getPayload().substring(0,4).equals("code")) {
			Array.add(message.getPayload().substring(4));
		}
		System.out.println(session.getId() + "로부터 메시지 수신 : " + message.getPayload());
		for(WebSocketSession s : users.values()) {
			if(Array.get(Integer.parseInt(s.getId(),16)).equals(Array.get(Integer.parseInt(session.getId(),16) ) ) ){
