...
public class BoardMatchCommand implements BoardCommand {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) {
		...
		ApplyDTO applydata = Adao.application(id);// 마지막 신청자 신청정보
		Stack<ApplyDTO> matchinglist = new Stack<ApplyDTO>();// 마지막 신청자 조건에 부합
		MemberDAO Udao = MemberDAO.getInstance();
		ArrayList<MemberBean> list = Udao.userlist();// 사용자정보리스트
		...
		for (MemberBean data : list) {
			...
			if (!(data.getGender().equals(applydata.getGender()))) {
				...
				if (count >= 4) {
					...
					dto = Adao.application(data.getId());
		...
		MemberBean Udto = new MemberBean();
		Udto = Udao.user(id);
		...
		while (!matchinglist.empty()) {// 스택 빌때까지
			...
			if (!(data.getGender().equals(Udto.getGender()))) {
				...
				// 매칭될 경우
				if (count >= 4) {
					matchinginfo = Udto.getId() + "님과 " + data.getId() + "님이 매칭되셨습니다.";
					int key;
					key=Udao.chatkey();
					Udao.setChatkey(Udto.getId(), key);
					Udao.setChatkey(data.getId(), key);
					// 매칭된 두 사람의 신청정보 삭제
					Adao.deleteApplication(Udto.getId());
					Adao.deleteApplication(data.getId());
					request.setAttribute("matchinginfo", matchinginfo);
					request.setAttribute("check", "true");
					request.setAttribute("chatkey", key);
					return;
				}
			}
			request.setAttribute("matchinginfo", matchinginfo);
